...
for xROI in range(0, len(ROI)):
    ...
    for xSN in range(0, len(sName)):
        ...
        result_acc_file = '%(result_path)s/%(Exp)s_%(roi)s_LMS2_decoding_0.8.csv' % {"Exp": exp, "result_path": result_path, "roi": ROI[xROI]}
        result_cfm_file = '%(result_path)s/%(Exp)s_%(roi)s_LMS2_cfm_0.8.csv' % {"Exp": exp, "result_path": result_path, "roi": ROI[xROI]}
        ...
        fds = fds[np.logical_or(fds.sa.color == 1, fds.sa.color == 2)]
        fds.sa['targets'] = fds.sa.LMS
        ...
        for xCond in range(0, len(cond)): # 1- in, 2 - out
            ...
            ######## SVM  ########
            nfeatures = ceil(fds_cond.nfeatures * 0.8)
            clf = LinearCSVMC()
    ...
    confusion_matrix = confusion_matrix.reshape(nTargets*len(cond), nTargets)
