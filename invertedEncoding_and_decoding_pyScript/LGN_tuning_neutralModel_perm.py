...
ROIs = ['V3_3dg_fmasked_q.001', 'V4v_fmasked_q.001']
...
for subjnum in subs:
    ...
    for i in ROIs:
        print i
        result_dir = 'perm_sc_dt_hp_am/' + i + '_2TRlag_neutralModel'
        ...
        permutator = AttributePermutator(['targets'], limit='ecc', count=nPerm)
        xPerm = 0
        for pm_fds in permutator.generate(fds):
            xPerm = xPerm+1
            print "Perm no. %d" % xPerm
            perm_csvname = 'perm' + str(xPerm) + '_' + subjnum + '_tuning_' + i + '2TR_used4TRs_neutralModel'
            print perm_csvname
            ...
            for rr in np.arange(1, nRun + 1):
                ...
                trn = pm_fds[pm_fds.sa.chunks != rr]  # data from training scans (all but one scan)
                tst = pm_fds[pm_fds.sa.chunks == rr]
                #trn = trn.get_mapped(averager)
                #tst = tst.get_mapped(averager)
            ...
            savetxt(os.path.join(result_dir, perm_csvname + '_unshift.txt'), stacked_channel_response_unshifted, '%f', '\t')
            savetxt(os.path.join(result_dir, perm_csvname + '_shift.txt'), stacked_channel_response_shifted, '%f', '\t')
