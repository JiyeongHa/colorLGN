...
for xROI in range(0, len(ROI)):
    ...
    for xSN in range(0, len(sName)):
        ...
        result_acc_file = '%(result_path)s/%(Exp)s_%(roi)s_8colors_decoding_used4TRs.csv' % {"Exp": exp, "result_path": result_path, "roi": ROI[xROI]}
        result_cfm_file = '%(result_path)s/%(Exp)s_%(roi)s_8colors_cfm_used4TRs.csv' % {"Exp": exp, "result_path": result_path, "roi": ROI[xROI]}
